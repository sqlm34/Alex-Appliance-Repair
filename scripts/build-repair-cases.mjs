...
import { fridgeStory, fridgeArticle } from './samsung-fridge-story.mjs';
...
export function buildRepairCases(){
 ...
 const cases=[fridgeStory,samsungStory,washerStory,lgRangeStory,...storedCases];
 ...
 editorialStories.set(fridgeStory.slug,fridgeArticle);
 const seen=new Set();
 ...
 for(const c of cases){
  ...
  const related=relatedPool.filter(x=>x.slug!==fridgeStory.slug&&x.slug!==samsungStory.slug&&x.slug!==c.slug&&x.slug!==washerStory.slug&&(c.slug===cafeSlug||x.slug!==cafeSlug)).sort((a,b)=>(b.appliance===c.appliance)-(a.appliance===c.appliance)).slice(0,3);
...
<section class="local-section local-section--soft"><div class="local-shell"><header class="local-section-header"><h2>More repair stories</h2><a href="/recent-work.html">View all repair stories</a></header><div class="case-grid">${related.map(c=>card(c)).join('')}</div></div></section>${cta}</main>`;
  ...
  if(editorialStories.has(c.slug)) {
   const editorialStyleVersion=c.slug===fridgeStory.slug||c.slug===samsungStory.slug||c.slug===cafeSlug||c.slug===washerStory.slug?'20260916-cafe-aligned':c.slug===fishersSlug?'20260911-highlighted-seal':'20260913-lg-dryer';
   ...
   if(c.slug!==fridgeStory.slug&&c.slug!==samsungStory.slug&&c.slug!==lgRangeStory.slug&&c.slug!==cafeSlug&&c.slug!==washerStory.slug) page=page.replace(/\b(href|src)="\/(?!\/)([^"]*)"/g,(_,attr,value)=>`${attr}="../${value||'index.html'}"`);
  }
