...
const title=c=>c.title+(c.city==='service-area'?'':' in '+cityName(c));
...
export function buildRepairCases(){
 const cases=JSON.parse(fs.readFileSync(path.join(ROOT,'scripts/repair-cases.json'),'utf8'));
 const seen=new Set();
 for(const c of cases){
  if(!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(c.slug)||seen.has(c.slug))throw Error('Invalid/duplicate case slug');seen.add(c.slug);
  if(!notes[c.slug]||!c.photos.length)throw Error('Missing editorial content: '+c.slug);
 ...
 for(const c of cases){
  ...
  const related=cases.filter(x=>x.slug!==c.slug).sort((a,b)=>(b.appliance===c.appliance)-(a.appliance===c.appliance)).slice(0,3);
...
<section class="local-section local-section--soft"><div class="local-shell"><header class="local-section-header"><h2>More repair stories</h2><a href="/recent-work.html">View all repair stories</a></header><div class="case-grid">${related.map(c=>card(c)).join('')}</div></div></section>${cta}</main>`;
  let page=shell.replace(/<main\b[\s\S]*?<\/main>/,main);
  page=metadata(styleLink(page),{name:title(c)+' | Alex Appliance Repair',description:c.summary,page:url(c),imageUrl:BASE+c.photos[0].src,type:'article',schema});
 ...
 const featured=[cases[0],...cases.filter(c=>c.city==='service-area').slice(0,5)];
 ...
 dates['blog.html']='2026-09-09';dates['recent-work.html']='2026-09-09';for(const c of cases)dates['repair-cases/'+c.slug+'.html']=c.modified;
